...
## 20 September 2026 — profile, authentication and Bluetooth update (1.0.7)

The profile heading fills its row and follows Arabic RTL alignment. Avatar controls use الصور with single-line labels. Profile balances use the supplied coin/gem/flame PNGs; password, deletion and sign-out actions occupy separate full-width rows. Local practice/puzzle summary counters are removed. Games uses square cards, smaller mode art and no arrows. Header counters use the owner's latest backgrounds and plus artwork, and the settings button matches the profile button height. Home uses start_game_icon.png. Puzzle guide art is rendered at 25% opacity; placed tiles stay fully opaque.

Server-assigned player numbers display as twelve ASCII digits with leading zeros. The same underlying unique ID is retained, so this does not replace identities or require a database migration. Profile/friend views have copy controls; friend lookup converts the twelve-digit display back to the server number. Verified IDs and usernames are cached for offline display. Device-only accounts receive their global ID after online registration; the client does not invent a searchable server identity.

Online login now authenticates with the server before accepting credentials or changing the offline cache. Failed online sign-in can no longer become a successful local login. Registration requiring email confirmation stays pending. Session restoration finishes before automatic connection work. Existing device-only accounts must register online once and confirm their email; existing server accounts sign in normally. A live sign-in of the configured existing account returned HTTP 200 with a confirmed email. No production schema, provider or confirmation settings changed. New-user email delivery was not independently exercised.

Added secure paired RFCOMM Bluetooth friendly quiz rooms: Duel 2 players, Duo 4, Squad 8, Solo 2–8; host-controlled clocks, ready checks, individual/team scoring, bounded versioned messages, connection/handshake/heartbeat timeouts, and socket cleanup. These are unranked quiz matches using bundled content, not the full internet mixed-minigame sequence. Physical multi-device Bluetooth testing remains outstanding; the emulator cannot validate radio links or controller connection limits.

Verification before handoff: 82 JVM tests and 25 Python tests passed; all 3,861 content records and all 30 current avatar/frame artwork files validated. The full emulator suite passed 43 tests, followed by 9 focused profile/header/Bluetooth-entry checks after lifecycle/cache refinements. Lint reports zero errors and 77 warnings. Arabic screenshots were visually inspected. Normal-package APK assembly and artifact integrity checks follow the requested local implementation commit. Android Studio is open on this project.

Guides: `docs/CONTENT_AUTHORING_GUIDE.md` covers image topics, choices/scoring, translated questions, exact puzzle cutting, validation, APK builds and filtered server exports. `tools/add_puzzle_content.py` generates all 96 coordinates and three localized records from previously cut images, refusing duplicate IDs. `docs/BLUETOOTH_PLAY.md` explains hosting/joining and the physical-device verification limit. Target installable handoff: `deliverables/BrainyBrawl-1.0.7.apk` (development signed).

Final artifact verification (21 September 2026): implementation commit `ce71f1e` was created before the normal APK build. Debug and optimized release assembly, all 82 JVM tests and lint succeeded. The installable development-signed APK is `deliverables/BrainyBrawl-1.0.7.apk`, application ID `com.brainybrawl.app`, version code 8, size 65,823,940 bytes, SHA-256 `22b63ae8d828f7c21a78b800aa59800d8ea9175c164ca08936070d94df8c4f3a`. V2 signature, ZIP/native 16 KB alignment, ZIP CRC, private-credential exclusion, all 24 original UI PNGs, two scenes and all 192 exact puzzle tiles passed verification.

The normal APK was installed in place, preserving Mr.onyx. The app restored the real online session, displayed permanent user ID `000010000000`, and opened Solo Online → Create private room → Choose your loadout without an account-connection prompt. No room or purchase was created during this check. No AndroidRuntime errors were recorded. The app remains open on Profile and Android Studio remains open on BrainyBrawl. The final short Arabic bottom-tab label and removal of the offline card arrow are included in this APK.
