...
## Owner clarification: shared scene, version 1.0.2

The `home` folder is the source for the background on every application page, not a route restriction. The shared shell now draws it once behind all destinations; individual pages no longer paint an opaque full-screen gradient over it. Selection remains stable during navigation and randomizes at the next app launch. Cards, the header and authentication captions retain themed surfaces for readable text. The splash still uses the separately selected splash scene. No additional database changes are needed.
