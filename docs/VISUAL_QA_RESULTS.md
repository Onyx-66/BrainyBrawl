...
Reviewed the new player card, account controls and Arabic profile under `.local/visual-qa/profile-customization/`. Avatar and frame use matching square geometry in the header/card. Currency/level chips share a row, Friends includes its count, and the account section groups the email edit, provider controls and password/deletion/sign-out actions. The old caption, gallery button and local-account explanation are absent. The first review found default tonal buttons looked disabled; explicit purple avatar/frame actions, navy password control and red-tinted deletion control corrected that visual ambiguity.

The full 36-test suite passed before this contrast adjustment, and three profile checks were rerun afterward. Save messages disappear after three seconds; tests preserve account data when deletion is cancelled. Arabic mirrors the card, currency strip and action layout. Native Lucide icons replace the prior hand-drawn navigation/action symbols. The twelve avatar/frame slots currently show numbered native preview art, not the final owner pack; final art requires a new visual pass after it is added.

Final review of the installed normal application confirmed the purple selector buttons, navy dialogs and all twelve labels fully visible (`picker-final.png`). The earlier short popup clipped the final label row; increasing the available grid height fixed it, and the three profile UI tests passed again. Seven appearance/profile checks also passed after the picker/metadata adjustment. The actual account Save reported cloud success; independent read-only inspection confirmed explicit avatar/frame IDs and a private JPEG. Existing account data was preserved.
