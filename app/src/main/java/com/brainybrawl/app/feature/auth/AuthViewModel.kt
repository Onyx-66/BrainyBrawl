...
class AuthViewModel(private val repository: AuthRepository) : ViewModel() {
    val onlineConfigured get()=repository.onlineConfigured
    ...
    fun submit(form: AuthForm, username: String, email: String, password: String,local:Boolean=false) = perform {
        when(form) {
            AuthForm.LOGIN -> if(local)repository.loginLocal(email,password)else repository.login(email,password)
            AuthForm.REGISTER -> if(local)repository.registerLocal(username,email,password)else repository.register(username,email,password)
